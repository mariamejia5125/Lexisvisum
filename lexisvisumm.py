...
def añadir_al_carrito(product_id):
    try:
        ...
        # Verificar si el producto ya existe en cart_items
        ...
        if existing:
            ...
            # plan_id va NULL porque es un producto, no una suscripción
...
def cart_count():
    try:
        cur = db.connection.cursor()
        cur.execute("SELECT SUM(quantity) FROM cart_items WHERE cart_id IN (SELECT id FROM carts WHERE user_id = %s)", (current_user.id,))
        result = cur.fetchone()
        cur.close()
...
def add_user_product():
  serial_number = request.form['serial_number']
    if serial_number=='LVK-':
      
    try:
        cur = db.connection.cursor()
        cur.execute("SELECT id FROM products WHERE serial_number = %s", (serial_number,))
        ...
        if product:
            ...
            db.connection.commit()
            flash('Producto añadido a tu cuenta', 'success')
        else:
            flash('Número de serie no encontrado', 'error')
